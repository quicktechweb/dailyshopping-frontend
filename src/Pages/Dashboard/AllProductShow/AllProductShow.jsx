...
const AllProductShow = () => {
  ...
  useEffect(() => {
    fetch(`https://dailyshopping-backend.onrender.com/allshowsproducts`) // Replace with your API URL
      .then((res) => res.json())
      .then((data) => {
  ...
  const handleApprove = (id) => {
    fetch(`https://dailyshopping-backend.onrender.com/approvedproducts/${id}`, {
      method: "PATCH",
      headers: {
  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios
          .delete(`https://dailyshopping-backend.onrender.com/pendingdatsdelete/${id}`)
          .then((response) => {
            response.status === 204 &&
