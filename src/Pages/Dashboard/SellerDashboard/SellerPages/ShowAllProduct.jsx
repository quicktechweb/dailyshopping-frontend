...
// 🆕 Search করলে পুরো product list থেকে খুঁজবে (শুধু current page না)
 // 🆕 Debounced backend search — সবসময় সার্ভার থেকে filtered + paginated রেজাল্ট আনে
  ...
  const delay = setTimeout(() => {
    ...
    setLoading(true);
    axios
      .get(`https://dailyshopping-backend.onrender.com/api/products/seller-search/${sellerId}`, {
 ...
 const fetchProducts = async (page = 1, limit = pageSize) => {
  try {
    ...
    const res = await axios.get(
      `https://dailyshopping-backend.onrender.com/api/products/seller-search/${sellerId}`,
      { params: { page, limit } }
    );
  ...
  {loading ? (
    ...
    filteredProducts.map((p) => (
      <tr key={p._id} className="hover:bg-gray-50">
        ...
        <td className="border p-2 whitespace-nowrap">{p.productid}</td>
