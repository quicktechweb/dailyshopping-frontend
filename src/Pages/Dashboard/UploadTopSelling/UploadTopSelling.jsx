...
const UploadTopSelling = () => {
  ...
  const fetchProducts = async () => {
    const res = await axios.get("https://dailyshopping-backend.onrender.com/api/topselling");
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      if (editingProduct) {
        await axios.put(
          `https://dailyshopping-backend.onrender.com/api/topselling/${editingProduct._id}`,
          productData
        );
        setEditingProduct(null);
      } else {
        await axios.post("https://dailyshopping-backend.onrender.com/api/topselling", productData);
      }
      setForm(emptyForm);
      setFiles([]);
      setPreviews([]);
  ...
  const handleDelete = async (id) => {
    if (window.confirm("Delete this product?")) {
      await axios.delete(`https://dailyshopping-backend.onrender.com/api/topselling/${id}`);
      fetchProducts();
    }
  };
    ...
    <div className="max-w-7xl mx-auto p-6 space-y-8">
      ...
      >
        <h2 className="text-2xl font-bold text-center">
          {editingProduct ? "Edit Product" : "Top Selling Product"}
        </h2>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <input
            ...
            className="border rounded-lg p-2"
            value={form.solds}
            onChange={(e) => setForm({ ...form, solds: e.target.value })}
          />
          <input
            type="text"
            placeholder="Save"
        ...
        <table className="w-full text-left border-collapse text-sm">
          <thead>
            <tr className="bg-gray-100">
              ...
              <th className="border p-2">Solds</th>
          ...
          <tbody>
            {currentProducts.map((p) => (
              <tr key={p._id} className="hover:bg-gray-50">
                ...
                <td className="border p-2">{p.sold}</td>
                <td className="border p-2">{p.shop}</td>
                <td className="border p-2">{p.totalcupon}</td>
                <td className="border p-2">{p.remaining}</td>
                <td className="border p-2">{p.solds}</td>
                <td className="border p-2">{p.save}</td>
...
export default UploadTopSelling;
