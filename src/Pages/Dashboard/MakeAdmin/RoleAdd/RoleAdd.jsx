...
export default function RoleManager() {
  ...
  const saveRole = async () => {
    ...
    try {
      if (editingRole) {
        const res = await axios.put(`https://dailyshopping-backend.onrender.com/roles/${editingRole._id}`, {
          name: roleName,
          permissions
        });
        ...
        const res = await axios.post("https://dailyshopping-backend.onrender.com/roles", {
          name: roleName,
          permissions
        });
  ...
  const deleteRole = async (id) => {
    ...
    if (confirm.isConfirmed) {
      await axios.delete(`https://dailyshopping-backend.onrender.com/roles/${id}`);
  ...
  return (
    <div className="max-w-4xl mx-auto mt-10 p-6 bg-white shadow-xl rounded-xl">
      ...
      <div className="flex gap-3 mb-6">
        ...
        {editingRole && <button className="px-4 py-2 bg-gray-300 rounded-lg" onClick={() => { setEditingRole(null); setRoleName(""); setPermissions({}); }}>Cancel</button>}
      </div>
